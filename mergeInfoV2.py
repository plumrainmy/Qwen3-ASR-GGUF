...
if __name__ == '__main__':
    path = './wav/13623465.wav'
    result = process_stereo_audio(path)
    left = result['left'].intervals
    right = result['right'].intervals

    res = processASR(path)
    print(res.alignment)
    merged_and_sort = merge_and_sort_intervals_with_text(
        left, right, res.alignment, res.alignment, left_label="客服：", right_label="客户：")
    print_sorted_intervals_with_text(merged_and_sort)
